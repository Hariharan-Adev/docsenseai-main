"""Reindex existing CSV documents into structured workbook tables."""
...
def _batch_size(value: str) -> int:
    ...
    if size > MAX_BATCH_SIZE:
        raise argparse.ArgumentTypeError(
            f"must not exceed {MAX_BATCH_SIZE}"
        )
    return size
...
def _organization_identifier(value: str) -> str:
    ...
    if not _ORGANIZATION_ID_PATTERN.fullmatch(identifier):
        raise argparse.ArgumentTypeError(
            "must be 1-128 letters, digits, dots, colons, underscores, or hyphens"
        )
    return identifier
...
def run_reindex(
    *,
    ...
    """Scan active CSVs and call the existing one-document reindex service."""
    started = monotonic()
    ...
    while True:
        ...
        for candidate in batch:
            ...
            if dry_run:
                continue
            try:
                result = reindex_existing_csv_document(
                    document_id=candidate.document_id,
                    owner_id=candidate.owner_id,
                    organization_id=candidate.organization_id,
                )
            except StructuredCsvReindexError:
                summary.failed += 1
                print(
                    json.dumps(
                        {
                            "document_id": candidate.document_id,
                            "status": "failed",
                            "error": "Structured CSV reindexing failed.",
                        },
                        sort_keys=True,
                    ),
                    file=sys.stderr,
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Reindex active stored CSV documents into structured workbook tables."
        )
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Report eligible documents without making database writes.",
    )
    parser.add_argument(
        "--document-id",
        type=_positive_identifier,
        help="Limit processing to one positive document ID.",
    )
    parser.add_argument(
        "--owner-id",
        type=_positive_identifier,
        help="Limit processing to one positive owner ID.",
    )
    parser.add_argument(
        "--organization-id",
        type=_organization_identifier,
        help="Limit processing to one validated organization ID.",
    )
    parser.add_argument(
        "--batch-size",
        type=_batch_size,
        default=100,
        help=f"Candidates read per batch (default: 100, maximum: {MAX_BATCH_SIZE}).",
    )
    parser.add_argument(
        "--retry-failed",
        action="store_true",
        help="Retry documents whose latest structured attempt failed.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Reindex matching documents regardless of status or index version.",
    )
    parser.add_argument(
        "--version",
        action="version",
        version=(
            ...
            f"(structured index {STRUCTURED_INDEX_VERSION})"
        ),
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            "Structured CSV reindexing could not access the configured database.",
            file=sys.stderr,
        )
        return 3
