...
def _is_overview_context_question(question: str) -> bool:
    """Recognize broad explanatory questions that need several related sections."""
...
def _is_narrow_fact_question(question: str) -> bool:
    """Recognize yes/no factual questions that should stay on one direct answer."""
    tokens = re.findall(r"[a-z0-9]+", question.casefold())
    return bool(tokens) and tokens[0] in {
        "are", "can", "could", "did", "do", "does", "is", "should",
        "was", "were", "will", "would",
    }
...
def _subject_tokens(question: str) -> set[str]:
    ...
    intent_tokens = {
        "about", "configuration", "configure", "describe", "document",
        "does", "explain", "how", "me", "overview", "setup", "tell", "the",
        "what", "work", "workflow",
    }
    return _evidence_tokens(question) - intent_tokens


def _subject_phrase(question: str) -> str:
    """Keep subject words in query order for exact heading phrase checks."""
...
def _leading_content(source: dict[str, object]) -> str:
    """Use early chunk text as a lightweight heading/title signal."""
    lines = [
        line.strip()
...
def _overview_relevance_score(question: str, source: dict[str, object]) -> int:
    """Boost overview sections whose leading text matches the requested subject."""
    subject_tokens = _subject_tokens(question)
    if not subject_tokens:
        return 0
    leading = _leading_content(source)
    full_text = " ".join((
        str(source.get("filename") or ""),
        str(source.get("content") or ""),
        str(source.get("source_location") or ""),
    ))
    leading_tokens = _evidence_tokens(leading)
    full_tokens = _evidence_tokens(full_text)
    leading_overlap = subject_tokens & leading_tokens
    full_overlap = subject_tokens & full_tokens
    required_overlap = min(len(subject_tokens), 2)
    if len(full_overlap) < required_overlap:
        return 0
    subject_phrase = _subject_phrase(question)
    normalized_leading = " ".join(re.findall(r"[a-z0-9]+", leading.casefold()))
    normalized_full = " ".join(re.findall(r"[a-z0-9]+", full_text.casefold()))
    score = (len(leading_overlap) * 4) + (len(full_overlap) * 2)
    if subject_phrase and subject_phrase in normalized_leading:
        score += 8
    elif subject_phrase and subject_phrase in normalized_full:
        score += 4
    primary_flow_tokens = {
        "apply", "build", "compare", "configure", "configuration", "create",
        "overview", "process", "setup", "workflow",
    }
    support_tokens = {"exception", "restriction", "step"}
    score += min(8, len(primary_flow_tokens & leading_tokens) * 4)
    score += min(8, len(primary_flow_tokens & full_tokens) * 4)
    score += min(2, len(support_tokens & full_tokens))
    return score


def _rank_overview_context(
    question: str,
    candidates: list[dict[str, object]],
) -> list[tuple[int, dict[str, object]]]:
    """Prefer section-title matches before falling back to retrieval rank."""
    return sorted(
        enumerate(candidates),
        key=lambda item: (
            -_overview_relevance_score(question, item[1]),
...
def select_final_context(
    ...
    overview_question = _is_overview_context_question(question) and not complex_question
    ranked = sorted(
        enumerate(candidates),
        key=lambda item: (
            -float(item[1].get("fusion_score") or item[1].get("score") or 0.0),
    ...
    if overview_question:
        ranked = _rank_overview_context(question, candidates)
    desired_count = (
        min(settings.rag_final_context_limit, 4)
        if overview_question
        else 1 if not complex_question else min(settings.rag_final_context_limit, 3)
    )
    selected: list[dict[str, object]] = []
    used_tokens = 0
    ...
    add(ranked[0][1])
    if not complex_question and _is_narrow_fact_question(question):
        return selected
    if overview_question:
        anchor = selected[0] if selected else ranked[0][1]
        for _, source in ranked[1:]:
            if (
                source.get("document_id") != anchor.get("document_id")
                or source.get("version_id") != anchor.get("version_id")
                or _overview_relevance_score(question, source) <= 0
            ):
                continue
